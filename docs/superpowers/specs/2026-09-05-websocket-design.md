...
- **선언 빈도 제어**: 5회/초 한도가 있으므로 짧은 코얼레싱 창(기본 100ms)을 두고 연속 호출을 한 번의 선언으로
  묶는다. `rate-limit-exceeded` 를 받으면 1초 후 1회 재선언한다. 이때 `Subscribe`/`Unsubscribe`/`Declare`
  호출을 실패로 풀어주지 않는다(`Errors()` 로만 통지) — 곧 나갈 재선언이 성공하면 호출자를 잘못된 실패로
  재시도시켜 한도를 더 악화시키기 때문이다.
