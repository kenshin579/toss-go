...
- 선언마다 `id` 를 채워 보내고(예: `d-7`), 같은 `id` 의 `subscriptions` ack 를 기다려 결과를 확정한다.
  ...
  낡은 ack 가 그 사이 사용자가 다시 넣은 구독을 지울 수 있기 때문이다.
  **`id` 없는 ack 는 어떤 대기 배치에도 매달지 않는다.** SDK 는 비어 있지 않은 선언에는 항상 `id` 를
  붙이므로, `id` 없는 ack 가 올 수 있는 유일한 경우는 `[]`(전체 해제) 뿐이고 그 경로는 자기 대기자를
  직접 해소한다 — "가장 오래 기다린 배치" 로 폴백하던 예전 구현은 그 폴백이 발화하는 유일한 상황에서
  항상 틀린(뒤이어 보낸 다른 선언의) 배치를 가로챘다.
...
- **선언 빈도 제어**: 5회/초 한도가 있으므로 짧은 코얼레싱 창(기본 100ms)을 두고 연속 호출을 한 번의 선언으로
  묶는다. 코얼레싱만으로는 그 창보다 넓게(하지만 여전히 잦게) 떨어진 연속 호출까지 막지 못하므로,
  최소 선언 간격(`minDeclareInterval`, 210ms — 문서 한도 200ms 에 여유)을 추가로 강제한다. 유휴 상태
  뒤의 첫 선언은 대기 없이 바로 나가고, 연속 선언만 조절된다. `rate-limit-exceeded` 를 받으면 1초 후
  1회 재선언한다. 이때 `Subscribe`/`Unsubscribe`/`Declare` 호출을 실패로 풀어주지 않는다(`Errors()`
  로만 통지) — 곧 나갈 재선언이 성공하면 호출자를 잘못된 실패로 재시도시켜 한도를 더 악화시키기
  때문이다.
- **full-replace 흡수**: 새 선언은 프로토콜상 앞서 보낸 선언을 전부 대체한다. 그래서 아직 자기 ack 를 못
  받은(예: 서버가 `id` 를 echo 하지 않는 `error` 프레임 뒤에 남겨진) 대기자는 새로 나가는 선언에 흡수돼,
  그 선언의 ack 가 대신 그들을 풀어준다 — 그렇지 않으면 `id` 가 영영 매칭되지 않아 호출자가 영원히 막힌다.
- 대기자 배치는 소켓에 쓰기 전에 등록한다 — ack 가 등록을 앞지르면 호출자가 영원히 막힌다.
  이 순서는 쓰기 직후에 걸리는 테스트 전용 훅(`config.afterDeclareWrite`)으로 창을 벌려 결정적으로 검증한다 — 실제 창이 매우 좁아 동시 부하만으로는 재현이 확률적이다.
...
- **flap guard**: 연결이 `backoffMin` 보다 짧게 유지됐으면 "성공한 연결"로 치지 않고 재연결 시도 횟수를
  리셋하지 않는다 — 즉시 첫 시도(0 대기)와 재연결 성공마다 리셋을 그대로 두면, 백프레셔처럼 클라이언트가
  스스로 끊는 경우 짧게 살다 죽는 연결이 반복될 때마다 매번 "새로 시작"으로 착각해 초당 수천 회 재다이얼
  하게 된다(실측: 기본 backoff 로 2초에 8949회). 서버가 끊은 정상 케이스의 즉시 복귀는 그대로 유지된다.
